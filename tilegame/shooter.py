...
IMAGE_SIZE = (30, 30)
BARREL_OFFSET = pg.math.Vector2(20, 10)
...
class Shooter(pg.sprite.Sprite):
	def __init__(self, game, x, y):
		...
		self.last_shot = 0
		self.health = 100
		self.health_bar = HealthBar(self.health, 50, 10)
	...
	def move(self):
		...
		if keys[pg.K_SPACE]:
			currect_time = pg.time.get_ticks()
			if currect_time - self.last_shot > BULLET_RATE:
				self.last_shot = currect_time
	...
	def update(self):
		...
		self.rect.center = self.pos
		self.health_bar.update(self.health, self.game.screen)
